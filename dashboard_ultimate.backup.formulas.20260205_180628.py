...
# Navigation
view = st.radio("Navigation", ["🗺️ Statewide Map", "📍 District View", "🚧 Live IDOT Map", "💰 Federal Funding", "📊 AI Analysis", "🤖 AV Policy"], horizontal=True)
...
# AI ANALYSIS
    ...
    with tab3:
        st.subheader("District-Level Allocation Estimates")
        st.markdown("*Based on STBG formula distribution*")
        ...
        df['Amount'] = df['Est. STBG'].str.replace('$','').str.replace(',','').astype(float)
        
        chart = alt.Chart(df).mark_bar().encode(
            x=alt.X('District:N', sort='-y'),
            y='Amount:Q',
            color='Type:N',
            tooltip=['District', 'Representative', 'Type', 'Est. STBG']
...
# FEDERAL FUNDING
    ...
    st.header("💰 Federal Funding")
    st.info("Federal funding tab temporarily disabled - being rebuilt")
